                ...
                widget.updateSliderForDirection = function(direction, splitPos) {
                    ...
                    switch(direction) {
                        ...
                        case 'diagonal':
                            ...
                            // Position handle diagonally with maximum visibility
                            this.sliderHandle.style.left = `${clipPercent}%`;
            ...
            nodeType.prototype.setupVideoInteraction = function(widget, videoContainer) {
                ...
                const updateSlider = (e) => {
                    ...
                    switch(direction) {
                        ...
                        case 'diagonal':
                            // For diagonal, calculate true diagonal distance based on container resolution
                            const dx = e.clientX - rect.left;
                            const dy = e.clientY - rect.top;
                            const normalizedX = dx / rect.width;
                            const normalizedY = dy / rect.height;
                            
                            // Calculate the actual diagonal distance from top-left corner
                            // Use Pythagorean theorem for true diagonal distance
                            const diagonalDistance = Math.sqrt(
                                Math.pow(normalizedX, 2) + Math.pow(normalizedY, 2)
                            );
                            
                            // The maximum diagonal distance is from corner to corner = sqrt(2)
                            const maxDiagonalDistance = Math.sqrt(2);
                            
                            // Scale to 0-1 range for full corner-to-corner coverage
                            splitPos = Math.max(0, Math.min(1, diagonalDistance / maxDiagonalDistance));
                            
                            console.log(`[Video_Compare_BMO] Diagonal calculation: x=${normalizedX.toFixed(3)}, y=${normalizedY.toFixed(3)}, distance=${diagonalDistance.toFixed(3)}, maxDistance=${maxDiagonalDistance.toFixed(3)}, splitPos=${splitPos.toFixed(3)}`);
                            break;
