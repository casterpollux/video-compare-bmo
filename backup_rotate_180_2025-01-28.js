                ...
                widget.updateSliderForDirection = function(direction, splitPos) {
                    ...
                    switch(direction) {
                        ...
                        case 'diagonal':
                            ...
                            // Calculate the exact diagonal length and angle based on container dimensions
                            ...
                            // Calculate the exact angle in degrees
                            // For a rectangle, the diagonal angle is atan(height/width)
                            const angleRadians = Math.atan(containerHeight / containerWidth);
                            const angleDegrees = angleRadians * (180 / Math.PI);
                            
                            this.sliderHandle.style.width = `${diagonalLength}px`;
                            this.sliderHandle.style.height = "3px";
                            this.sliderHandle.style.borderRadius = "0";
                            ...
                            // Use the calculated angle (positive for top-left to bottom-right)
                            this.sliderHandle.style.transform = `translate(-50%, -50%) rotate(${angleDegrees}deg)`;
                            this.videoContainer.style.cursor = "nwse-resize";
                            
                            console.log(`[Video_Compare_BMO] Diagonal: ${containerWidth}x${containerHeight}, length: ${diagonalLength.toFixed(1)}px, angle: ${angleDegrees.toFixed(1)}°`);
