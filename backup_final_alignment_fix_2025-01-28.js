                ...
                widget.updateSliderForDirection = function(direction, splitPos) {
                    ...
                    switch(direction) {
                        ...
                        case 'diagonal':
                            ...
                            // --- CORRECTED HANDLE LOGIC ---

                            // This logic correctly positions the handle's center on the midpoint of the 45-degree edge.
                            const handleX = clipPercent;
                            const handleY = clipPercent;

                            this.sliderHandle.style.left = `${handleX}%`;
                            this.sliderHandle.style.top = `${handleY}%`;

                            // Get container dimensions to set the line's length correctly.
                            const containerRect = this.videoContainer.getBoundingClientRect();
                            const diagonalLength = Math.sqrt(containerRect.width**2 + containerRect.height**2);
                            
                            this.sliderHandle.style.width = `${diagonalLength}px`;
                            this.sliderHandle.style.height = "2px"; // A thin line
                            this.sliderHandle.style.borderRadius = "0";
                            this.sliderHandle.style.backgroundColor = "rgba(255, 255, 255, 1.0)";
                            this.sliderHandle.style.border = "1px solid rgba(0, 0, 0, 0.8)";
                            this.sliderHandle.style.boxShadow = "0 0 6px rgba(255, 255, 255, 0.8), 0 0 3px rgba(0, 0, 0, 0.8)";
                            this.sliderHandle.style.zIndex = "30";
                            this.sliderHandle.style.display = "block";
                            this.sliderHandle.style.visibility = "visible";
                            this.sliderHandle.style.opacity = "1";
                            
                            // THE FIX: Rotate the line by a FIXED -45 degrees to match the clip-path.
                            // We do not calculate a dynamic angle. -45deg aligns perfectly with the polygon edge.
                            this.sliderHandle.style.transform = `translate(-50%, -50%) rotate(-45deg)`;
                            
                            this.videoContainer.style.cursor = "nwse-resize";
                            console.log(`[Video_Compare_BMO] Applied corrected diagonal slider - angle: -45deg`);
                            break;
